...
export async function GET() {
  try {
    ...
    console.log('Foundry assistants fetched successfully')
    return NextResponse.json(result)
  } catch (error) {
    console.error('Error fetching Foundry assistants:', error)
    return NextResponse.json(
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log('Creating Foundry assistant with payload:', JSON.stringify(foundryPayload, null, 2))
    ...
    const result = await response.json()
    console.log('Foundry assistant created successfully:', result.id)
    return NextResponse.json(result)
  } catch (error) {
    console.error('Error creating Foundry assistant:', error)
    return NextResponse.json(
      { error: 'Internal server error', details: error.message },
      { status: 500 }
    )
